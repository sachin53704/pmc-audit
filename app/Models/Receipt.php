...
class Receipt extends BaseModel
{
    ...
    {
        return $this->hasMany(SubReceipt::class);
    }
    ...
    {
        static::created(function (Receipt $object)
        {
            if(Auth::check())
            {
        ...
        static::updated(function (Receipt $object)
        {
            if(Auth::check())
            {
                self::where('id', $object->id)->update([
        ...
        static::deleting(function (Receipt $object)
        {
            if(Auth::check())
            {
                self::where('id', $object->id)->update([
