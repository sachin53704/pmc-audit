...
class ReportController extends Controller
{
    ...
    public function paraAuditReport(Request $request)
    {
        $audits = Audit::query()
            ->when(Auth::user()->hasRole('Auditor'), function($q){
                $q->withCount([
                    'objections as unapproved' => fn ($q) => $q->where('auditor_action_status', 0)->whereNull('answer')->where('user_id', Auth::user()->id),
                ]);
            })->when(Auth::user()->hasRole('MCA') || Auth::user()->hasRole('DY MCA') || Auth::user()->hasRole('Super Admin') || Auth::user()->hasRole('Admin'), function($q){
                $q->withCount([
                    'objections as unapproved' => fn ($q) => $q->whereNull('answer')->where('auditor_action_status', 0),
                ]);
            })
            // ->where('department_id', Auth::user()->department_id)
            ->latest()
    ...
    public function complienceAnswerReport(Request $request){
        $audits = Audit::query()
            ->when(Auth::user()->hasRole('Auditor'), function($q){
                ...
                $q->withCount([
                    'objections as approved' => fn ($q) => $q->where('auditor_action_status', 1),
                ]);
            })
            // ->where('department_id', Auth::user()->department_id)
            ->latest()
    ...
    public function getResponseQuestion(Request $request){
        if($request->ajax()){
            ...
            ->when(Auth::user()->hasRole('Auditor'), function($q) use($request){
                ...
                $q->where([
                    'audit_id' => $request->id,
                    'mca_action_status' => $request->status
                ]);
            })->get();
    ...
    public function departmentWiseProgramAudit(Request $request){
        ...
        $objections = Department::when(Auth::user()->hasRole('Auditor'), function($q) use($request){
            ...
            $q->withCount([
                'auditObjections as unapproved' => fn ($q) => $q->whereNull('answer')->where('auditor_action_status', 0),
                'auditObjections as approved' => fn ($q) => $q->where('auditor_action_status', 1),
            ]);
        })->get();
