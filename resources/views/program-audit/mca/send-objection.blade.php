<x-admin.layout>
    <x-slot name="title">HMM Draft</x-slot>
    <x-slot name="heading">HMM Draft</x-slot>
    {{-- <x-slot name="subheading">Test</x-slot> --}}

    @forelse($audits as $auditss)
    <div class="row">
        <div class="col-lg-12">
            <form action="" class="addForm" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        @php
                            $ids[] = $audit->id;
                            $hmmDraftNo = $audit->hmm_draft_number;
                            if(Auth::user()->hasRole('DY MCA')){
                                $hmmDraftCurrentStatus = $audit->hmm_draft_dymca_status;
                                $hmmDraftCurrentRemark = $audit->hmm_draft_dymca_remark;
                            }
                            elseif(Auth::user()->hasRole('MCA')){
                                $hmmDraftCurrentStatus = $audit->hmm_draft_mca_status;
                                $hmmDraftCurrentRemark = $audit->hmm_draft_mca_remark;
                            }
                        @endphp
                        @endforeach
                        <div>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            <input type="hidden" name="hmm_draft_number" value="{{ $hmmDraftNo }}">
                            <div class="col-6">
                                <label for="hmm_draft_dymca_status" class="form-label">Status <span class="text-danger">*</span></label>
                                <select name="hmm_draft_dymca_status" class="form-select" id="hmm_draft_dymca_status" required>
                                    <option value="">Select</option>
                                    <option value="1" @if($hmmDraftCurrentStatus == "1")selected @endif>Approve</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <div class="mb-3">
                                    <label for="hmm_draft_dymca_remark" class="form-label">Remark</label>
                                    <textarea name="hmm_draft_dymca_remark" id="hmm_draft_dymca_remark" class="form-control">{{ $hmmDraftCurrentRemark }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <button class="btn btn-primary" type="submit" style="padding: 6px 8px 6px 10px">
                            <i data-feather="send"></i> @if(Auth::user()->hasRole('DY MCA'))Send @else Send To Department HOD @endif
                        </button>
                    </div>
                    @endif
                </div>
